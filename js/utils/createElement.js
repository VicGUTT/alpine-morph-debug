...
/**
 * @param {Element} from
 * @param {Element|DocumentFragment|string} to
 * @returns {Element}
 */
export default function createElement(from, to) {
    // return document.createRange().createContextualFragment(to).firstElementChild;

    const DOCUMENT_FRAGMENT_NODE = 11;

    if (['#document', 'HTML', 'BODY'].includes(from.nodeName) && typeof to === 'string') {
